...
class ZRAService {
    ...
    async createSmartInvoice(invoice) {
        ...
        if (process.env.NODE_ENV !== 'production') {
            ...
            const processedItems = invoice.items.map(item => (Object.assign(Object.assign({}, item), { vatAmount: item.unitPrice * item.quantity * (item.vatRate / 100), total: item.unitPrice * item.quantity * (1 + item.vatRate / 100) })));
